                                ...
                                <div class="d-flex flex-wrap justify-content-between align-items-center details">
                                    ...
                                    <div>
                                        <a href="javascript:void(0)"
                                           class="btn btn-white text-primary btn-recharge fw-600">{{ trans('web.Recharge') }}</a>
                                    </div>
                                </div>
                            </div>
                            <div class="wallet-recharge d-none">
                                <div class="card mb-3">
                                    <p class="fw-600 mb-1">{{ trans('web.Recharge') }} :</p>
                                    @if($cards->count())
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0">
                ...
                <div class="modal-body p-0">
                    ...
                    {{ html()->form('post', route('web.wallet.recharge'))->open() }}
                    <div class="d-flex justify-content-between mb-3">
                        <input id="amount" class="form-control" type="number" required min="1"
                               placeholder="{{ trans('web.Amount') }}" name="amount" value="{{ old('amount') }}"/>
                        {{ html()->hidden('payment_method')->id('paymentMethodInput') }}
                    </div>
                    @if($errors->wallet->has("amount"))
                        <div class="error-note">
                            <span class="help-block text-danger">{{ $errors->wallet->first("amount") }}</span>
                        </div>
                    @endif
                    @if($errors->wallet->has("payment_method"))
                        <div class="error-note">
                            <span class="help-block text-danger">{{ $errors->wallet->first("payment_method") }}</span>
                        </div>
                    @endif
