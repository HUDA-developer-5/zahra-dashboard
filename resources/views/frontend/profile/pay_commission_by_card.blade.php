                            ...
                            <div class="card-gradient mb-3">
                                <div class="d-flex flex-wrap justify-content-between align-items-center details">
                                    <div class="info">
                                        ...
                                        <p class="fs-5 fw-bold mb-0">{{ $totalAmount }} {{ \Illuminate\Support\Str::upper(auth('users')->user()->default_currency) }}</p>
                                    </div>
                                    <div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0">
                ...
                <div class="modal-body p-0">
                    ...
                    {{ html()->form('post', route('web.pay.payCommissionWithCard'))->open() }}
                    <div class="d-flex justify-content-between mb-3">
                        <input id="amount" class="form-control" type="number" disabled
                               placeholder="{{ trans('web.Amount') }}" name="amount" value="{{ $totalAmount }}"/>
                        {{ html()->hidden('payment_method')->id('paymentMethodInput') }}
                        {{ html()->hidden('id')->value($commissionId) }}
                    </div>
                    @if($errors->commission->has("payment_method"))
                        <div class="error-note">
                            <span class="help-block text-danger">{{ $errors->commission->first("payment_method") }}</span>
                        </div>
                    @endif
